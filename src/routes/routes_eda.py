...
@app_eda.route('/x_y_columns/<graph_selected>', methods=['GET', 'POST'])
def x_y_columns(graph_selected):
    try:
        if 'pid' in session:
            ...
            if df is not None:
                ...
                if graph_selected == "Bar Graph":
                    return render_template('eda/x_y_columns.html', columns=list(df.columns), x_list=list(df.columns), y_list=num_cols, graphJSON=graphJSON)
